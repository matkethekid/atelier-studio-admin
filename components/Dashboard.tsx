...
const recentActivities = [
  {
    id: 0,
    text: "Dodat novi predavac",
    date: Date.now() - 13 * 2592000 * 1000,
  },
];
            ...
            recentActivities.map((activity, index: number) => (
              <li key={index} className="flex flex-row gap-20 items-center">
                <div className="flex flex-row gap-3 items-center">
                  <div className="p-2 flex items-center justify-center rounded-lg bg-blue-500/20 border border-blue-500/25">
                    <Clock size={20} />
                  </div>{" "}
                  {activity.text}
                </div>
                <span className="text-zinc-600 text-sm">
                  {calculateTimeElapsed(activity.date, end)}
                </span>
              </li>
            ))
          ) : (
